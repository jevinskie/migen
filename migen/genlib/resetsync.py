...
__all__ = ["AsyncResetSynchronizer", "AsyncResetSingleStageSynchronizer"]
...
class AsyncResetSingleStageSynchronizer(Special):
    """AsyncResetSingleStageSynchronizer
    Connects a synchronized reset signal to the provided ClockDomain. Synchronizing is done with two
    D-Flip-Flops in the provided ClockDomain. The output of the second flip-flop is directly connected
    to the reset signal of the provided ClockDomain.

    Asynchronous rising edge of the async_reset signal propagates directly to the output. Clearing the
    async_reset signal is synchronized by the *SINGLE* flip-flop. Thus maybe some metastability mumbo
    jumbo but it's supposed to be used for SPI CS# and YMMV YOLO!
    ...
    @staticmethod
    def lower(dr):
        raise NotImplementedError("Attempted to use a single-stage reset synchronizer, but platform does not support them")
